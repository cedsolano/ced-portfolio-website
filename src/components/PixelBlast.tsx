...
void main(){
  ...
  if(uEnableRipples==1){
    for(int i=0;i<MAX_CLICKS;++i){
      ...
      float waveR=speed2*t;float ring=exp(-pow((r-waveR)/thickness,2.0));
  ...
  useEffect(() => {
    ...
    if (mustReinit) {
      if (threeRef.current) {
        ...
        (t.quad?.geometry as THREE.BufferGeometry)?.dispose();
        t.material.dispose();
        t.composer?.dispose();
        t.renderer.dispose();
      ...
      const animate = () => {
        ...
        if (liquidEffect) {
          const liqU = (liquidEffect as Effect & { uniforms: Map<string, THREE.Uniform> }).uniforms;
          liqU.get('uTime')!.value = uniforms.uTime.value;
        }
      ...
      threeRef.current = { renderer, scene, camera, material, clock, clickIx: 0, uniforms, resizeObserver: ro, raf, quad, timeOffset, composer, touch, liquidEffect, onPointerDown, onPointerMove };
    } else {
      const t = threeRef.current!;
      const u = t.uniforms;
      (u.uShapeType.value as number) = SHAPE_MAP[variant] ?? 0;
      u.uPixelSize.value = pixelSize * t.renderer.getPixelRatio();
